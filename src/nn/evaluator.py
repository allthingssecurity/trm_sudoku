...
from src.nn.data.arc_datamodule import (
    ARCTaskDataset,
    TRMTransform,
    collate_fn_with_puzzles as collate_fn,
)
...
class ARCEvaluator:
    ...
    def evaluate_batch(self, batch: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        """Evaluate a single batch for TRM or ConvMLP models."""
        # Move tensors to device
        batch = {k: v.to(self.device) if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
        targets = batch["output"]

        with torch.no_grad():
            if isinstance(self.model, TRMModule):
                # Run TRM for up to N_supervision steps and take final logits
                ...
                preds_flat = last_outputs["logits"].argmax(dim=-1)

                # Reshape to grid to match targets
                if targets.dim() == 3:
                    b, h, w = targets.shape
                    predictions = preds_flat.view(b, h, w)
                else:
                    predictions = preds_flat

            else:
                # Fallback for simple models that accept tensors directly
                predictions = self.model(batch["input"]).argmax(dim=-1)

            # Calculate metrics
            correct = (predictions == targets).float()

            # Mask out padding (TRMTransform default pad_value = 10)
            mask = (targets != 10).float()

            # Pixel-level accuracy
            pixel_accuracy = (
                (correct * mask).sum() / mask.sum() if mask.sum() > 0 else correct.mean()
            )
