...
class TRMModule(LightningModule):
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor):
        # Ensure input is tokenized as [batch, seq_len]
        # Accept either [B, H, W] or [B, seq_len]
        if input.dim() == 3:
            b, h, w = input.shape
            input = input.view(b, h * w)
        elif input.dim() != 2:
            raise RuntimeError(f"Expected input with 2 or 3 dims, got shape {tuple(input.shape)}")

        # Token embedding -> [B, seq_len, hidden]
        ...
        if self.hparams.puzzle_emb_dim > 0:
            ...
            # Prefix puzzle embedding tokens along the sequence dimension
            embedding = torch.cat(
                (
    ...
    def compute_loss_and_metrics(self, carry, batch):
        ...
        # Flatten labels to [B, seq_len] if provided as [B, H, W]
        if labels.dim() == 3:
            b, h, w = labels.shape
